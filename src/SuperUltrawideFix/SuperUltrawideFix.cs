...
[assembly: MelonInfo(typeof(SuperUltrawideFix.SuperUltrawideFixMod), "Super Ultrawide Fix", "1.0.0", "snoozeds")]
...
namespace SuperUltrawideFix
{
    public class SuperUltrawideFixMod : MelonMod
    {
        public override void OnApplicationStart()
        {
            ...
            var getAttrMethod = typeof(SupportedResolutionTypes).GetMethod("GetAttr",
                BindingFlags.Public | BindingFlags.Static);

            if (getAttrMethod != null)
            {
                harmony.Patch(getAttrMethod,
                    postfix: new HarmonyMethod(typeof(SupportedResolutionTypesPatch), "GetAttrPostfix"));
                Melon<SuperUltrawideFixMod>.Logger.Msg("Successfully patched SupportedResolutionTypes.GetAttr");
            }
            else
            {
            ...
            var getClosestResMethod = typeof(SupportedResolutionTypes).GetMethod("GetClosestRes",
                new Type[] { typeof(int), typeof(int) });

            if (getClosestResMethod != null)
            {
                harmony.Patch(getClosestResMethod,
                    postfix: new HarmonyMethod(typeof(SupportedResolutionTypesPatch), "GetClosestResPostfix"));
                Melon<SuperUltrawideFixMod>.Logger.Msg("Successfully patched SupportedResolutionTypes.GetClosestRes");
            }
        }
    }
    ...
    {
        public static void GetAttrPostfix(SupportedResolutionType fromType, ref SupportedResolutionTypeAttr __result)
        {
            switch (fromType)
            {
                case SupportedResolutionType.RES_3440_X_1440:
                    // Override 21:9 3440x1440 with 32:9 5120x1440
                    __result = CreateCustomAttribute(ResolutionType.RES_21_9, 5120, 1440, __result.SaveID);
                ...
                case SupportedResolutionType.RES_3840_X_2160:
                    ...
                    __result = CreateCustomAttribute(ResolutionType.RES_16_9, 7680, 2160, __result.SaveID);
                    Melon<SuperUltrawideFixMod>.Logger.Msg("Overriding 3840x2160 → 7680x2160");
                    break;
            }
        }

        public static void GetClosestResPostfix(int width, int height, ref SupportedResolutionType __result)
        {
        ...
        private static SupportedResolutionTypeAttr CreateCustomAttribute(ResolutionType resType, int width, int height, int saveID)
        {
            ...
            return (SupportedResolutionTypeAttr)constructor.Invoke(new object[] { resType, width, height, saveID });
        }
    }
}